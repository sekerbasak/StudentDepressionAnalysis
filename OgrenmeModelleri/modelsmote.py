...
from imblearn.over_sampling import SMOTE  # SMOTE'u ekliyoruz
...
def main():
    ...
    models = {
        "Logistic Regression": {
            ...
            "params": {
                'penalty': ['l1', 'l2', 'elasticnet'],  # 'none' yerine 'l2' veya 'l1' kullanmalısınız
                'C': [0.01, 0.1, 1, 10],
                'solver': ['liblinear', 'lbfgs', 'saga']  # Uyumlu solver'lar
            }
        },
        "Decision Tree": {
    ...
    for model_name, config in models.items():
        ...
        grid_search.fit(X_train_res, y_train_res)  # SMOTE uygulanmış veriyi kullanıyoruz

        best_model = grid_search.best_estimator_
        best_params = grid_search.best_params_
        print(f"En iyi hiperparametreler: {best_params}")

        report, metrics = evaluate_model(best_model, X_train_res, y_train_res, X_test, y_test)
    ...
    plot_roc_curve(best_models, X_test, y_test, filename="roc_curvesmote.png")

    # Eğitim ve test doğruluğu histogramını kaydet
    plot_accuracy_histogram(results, filename="accuracy_histogramsmote.png")
