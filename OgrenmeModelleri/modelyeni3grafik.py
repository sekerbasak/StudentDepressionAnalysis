...
def plot_metrics_comparison(results, filename="metrics_comparison.png"):
    models = list(results.keys())

    # Metrikler: Precision, Recall ve F1-Score
    ...
    # Precision (turuncu)
    plt.bar(index, precision_scores, bar_width, color='orange', label='Precision')

    # Recall (yeşil)
    plt.bar([i + bar_width for i in index], recall_scores, bar_width, color='green', label='Recall')

    # F1-Score (mavi)
    plt.bar([i + bar_width*2 for i in index], f1_scores, bar_width, color='blue', label='F1-Score')
...
def main():
    ...
    plot_roc_curve(best_models, X_test, y_test, filename="roc_curveson.png")

    # Eğitim ve test doğruluğu histogramını kaydet
    plot_accuracy_histogram(results, filename="accuracy_histogramson.png")

    # Precision, Recall ve F1-Score karşılaştırmasını içeren grafiği kaydet
    plot_metrics_comparison(results, filename="metrics_comparison.png")
